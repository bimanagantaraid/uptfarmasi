...
class Satelit extends CI_Controller
{
    ...
    function tambahtransaksi(){
        ...
        if($this->form_validation->run() == false){
            ...
            $TanggalStart = ($Tanggal[1]-1);
            $start_date = "$Tanggal[0]-$TanggalStart-26";
            $end_date = "$Tanggal[0]-$Tanggal[1]-25";
            $date_check = $_POST['Tanggal'];
            $valid = $this->check_in_range($start_date, $end_date, $date_check);
            var_dump($valid);
            var_dump($start_date);
            var_dump($end_date);
            var_dump($date_check);
            var_dump($TanggalFilter);
            var_dump($Tanggal);
            if(($sisaStok - ($MutasiKeluar + $MutasiRusak)) < 0){
                ...
                echo json_encode($datakembali);
            }else if($Tanggal[0] != $TanggalFilter[1] && $Tanggal[1] != $TanggalFilter[0]){
                $datakembali = array(
                    'keterangan'    => 'datamiss',
                    'tanggal'       => $Tanggal,
